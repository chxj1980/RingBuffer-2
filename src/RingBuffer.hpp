...
public:
    ...
    using uniqueue_ptr = std::unique_ptr<RingBuffer>;
    ...
    inline size_t GetCapacityBytes() const {
    ...
    struct Buffer {
         ...
         */
        Buffer(uint8_t *buffer, size_t bufferCapacity) : mBuffer(buffer), mBufferCapacity(bufferCapacity) {}
    };
