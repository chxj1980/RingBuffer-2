...
//  RingBuffer
//
//  Created by SuXinDe on 2021/1/24.
...
bool RingBuffer::Allocate(size_t capacityBytes) {
    Deallocate();
...
size_t RingBuffer::GetBytesAvailableToRead() const {
...
size_t RingBuffer::GetBytesAvailableToWrite() const {
    auto w = mWritePointer;
    auto r = mReadPointer;
...
size_t RingBuffer::Read(void *destinationBuffer, size_t byteCount) {
...
size_t RingBuffer::Peek(void *destinationBuffer, size_t byteCount) const
{
    if(nullptr == destinationBuffer || 0 == byteCount) {
        return 0;
    }
...
size_t RingBuffer::Write(const void *sourceBuffer, size_t byteCount)
{
...
void RingBuffer::ReadAdvance(size_t byteCount) {
    mReadPointer = (mReadPointer + byteCount) & mCapacityBytesMask;
}

void RingBuffer::WriteAdvance(size_t byteCount) {
    mWritePointer = (mWritePointer + byteCount) & mCapacityBytesMask;;
}

RingBuffer::BufferPair RingBuffer::GetReadVector() const {
    auto w = mWritePointer;
    auto r = mReadPointer;
...
RingBuffer::BufferPair RingBuffer::GetWriteVector() const {
    auto w = mWritePointer;
    auto r = mReadPointer;
